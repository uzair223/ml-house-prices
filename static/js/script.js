...
function formatNumber(n, precision) {
  for (var i = 0; i < ranges.length; i++) {
    if (n >= ranges[i].divider) {
      return (
        (n / ranges[i].divider).toPrecision(precision).toString() +
        ranges[i].suffix
      );
    }
  }
...
function submitForm(e) {
  ...
  $.ajax({
    ...
    success: (resp) => {
      coords = { lat: resp.result.latitude, lng: resp.result.longitude };
    },
    error: handleError,
  });

  // setting up model parameters with 75% prediction interval
  const params = { ...fields, ...coords, alpha: 0.25 };
  ...
  $.ajax({
    ...
    success: (res) => {
      ...
      $("#output__point").text(
        "£" + parseInt(Math.round(res.point / 100) * 100).toLocaleString()
      ); // to nearest hundred
